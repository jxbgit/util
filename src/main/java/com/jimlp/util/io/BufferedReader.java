...
public class BufferedReader extends Reader {
     ...
     * @throws IOException
     */
    String readLine(char[] eolSign) throws IOException {
        int eolSignLength = eolSign.length;
        if(cb.length < eolSignLength) {
            throw new IllegalArgumentException("Buffer size < End of line sign size");
        }
        StringBuffer s = null;
        int startChar;
        int eolSignMark = 0;
        ...
        synchronized (lock) {
            ...
            for (;;) {
                ...
                charLoop: for (i = nextChar; i < nChars; i++) {
                    c = cb[i];
                    if(c!=eolSign[eolSignMark]){
                        eolSignMark = 0;
                    }
                    if(c==eolSign[eolSignMark]){
                        if(off == 0 || i-eolSignMark == off){
                            off = i-eolSignMark;
                        }
                        eolSignMark++;
                    }
                    if(eolSignMark >= eolSignLength){
                        eol = true;
                        i++;
                        break charLoop;
                    }
                ...
                if (s == null)
                    s = new StringBuffer(defaultExpectedLineLength);
                if (eolSignMarkIf != 0 && nextChar > eolSignLength - eolSignMarkIf) {
                    s.append(eolSign, 0, eolSignMarkIf);
                }
                
                if (eol) {
                    try {
                        s.append(cb, startChar, i - startChar - eolSign.length);
                ...
                s.append(cb, startChar, i - startChar - eolSignMark);
                eolSignMarkIf = eolSignMark;
            }
        }
    }
    /**
     ...
     * {@code BufferedReader}. The {@link Stream} is lazily populated, i.e.,
     * read only occurs during the
     ...
     * which will be thrown from the {@code Stream} method that caused the read
     * to take place. This method will return a Stream if invoked on a
