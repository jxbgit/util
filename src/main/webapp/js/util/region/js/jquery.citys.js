...
(function (factory) {
    ...
    $.fn.citys = function(parameter,getApi) {
        ...
        var defaults = {
            dataUrl:'../region/json/region/list.json',     //数据库地址
                    ...
                    var format = {
                        ...
                        city:function(){
                            ...
                            if(!hasCity){
                                ...
                                if(options.nodata=='disabled'){
                                    ...
                                    $city.css('display',$.isEmptyObject(city)?'none':'inline-block');
                                }
                                for(var i in city){
                        ...
                        area:function(){
                            ...
                            if(options.nodata=='disabled'){
                                ...
                                $area.css('display',$.isEmptyObject(area)?'none':'inline-block');
                            }
                            for(var i in area){
...
function getRegionNameByCode(code){
	...
	$.ajax({
        ...
        success:function(data){
			 var regionCode=code;
			 if(regionCode){   //如果有地区编码
				 var c = regionCode - regionCode%1e4;
				 if(data[c]){
				     province = data[c];
				 }else{province = "";}
				 
				 c = regionCode - (regionCode%1e4 ? regionCode%1e2 : regionCode);
				 if(data[c]){
				     city = data[c];
				 }else{city = "";}
				 
				 c = regionCode%1e2 ? regionCode : 0;
				 if(data[c]){
				     area = data[c];
				 }else{area = "";}
			 }
