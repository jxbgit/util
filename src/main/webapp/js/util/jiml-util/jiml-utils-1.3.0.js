 ...
 * time：2017/11/03
 * V 1.2.3 -> V 1.3.0
 * 添加 daysAgoOf(time) 方法，显示过去某个时间点到现在相差的天数等
 */
/////////格式化时间 start////////////////////////////////
/**
 * 格式化时间
 ...
 * @param time 某个时间的毫秒数
 */
function daysAgoOf(time){
	var date3=new Date().getTime()-time;  //时间差的毫秒数
	//计算出相差天数
...
function showMessage(data) {
	...
	$("#jiml_showMessage").css({"position":"fixed","top":messageTop+"px","left":messageLeft+"px","background-color":"rgba(0, 153, 255, 0.8)","color":"white","border-radius":"5px","font-size":fontSize+"px","padding":fontSize/4+"px","z-index":"9999999", "display":"none"});
