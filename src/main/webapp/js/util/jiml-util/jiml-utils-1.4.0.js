 ...
 * time：2017/11/03
 * V 1.3.0 -> V 1.4.0
 * daysAgoOf(time) 方法 -> timeGap(time1, time2)，
 * 分页样式添加行高控制，
 * 其他优化。
 */
...
function showMessage(data) {
	...
	var messageLeft = data.left!=undefined?data.left:w/2-messageLength*fontSize/2-fontSize/4;
	...
	$("#jiml_showMessage").css({"position":"fixed","background-color":"rgba(0, 153, 255, 0.8)","color":"white","border-radius":"5px","font-size":fontSize+"px","padding":fontSize/4+"px","z-index":"9999999", "display":"none"});
	var messageTop = data.top!=undefined?data.top:h/3;
	var messageLeft = data.left!=undefined?data.left:w/2-$("#jiml_showMessage").outerWidth()/2;
	$("#jiml_showMessage").css("top",messageTop);
