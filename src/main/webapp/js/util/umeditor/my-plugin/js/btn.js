...
UM.registerUI('textindent', function( name ){
    ...
    var me = this,
        ...
        $button = $.eduibutton({
            //按钮icon的名字， 在这里会生成一个“edui-icon-textIndent”的className的icon box，
            //用户可以重写该className的background样式来更改icon的图标
            //覆盖示例见btn.css
            'icon': 'textindent',
            'title': me.options.lang === "zh-cn" ? "首行缩进" : "textindent",
            'click': function(){
                //在这里处理按钮的点击事件
                //点击之后执行textIndent命令
                me.execCommand( name );
